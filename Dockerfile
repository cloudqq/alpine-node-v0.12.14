...
RUN apk add --no-cache curl make gcc g++ python linux-headers paxctl libgcc libstdc++ gnupg mariadb-dev && \
  ...
  apk del curl make gcc g++ python linux-headers paxctl gnupg mariadb-dev ${DEL_PKGS} && \
